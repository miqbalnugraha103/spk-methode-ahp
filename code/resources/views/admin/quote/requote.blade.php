...
@section('title', 'Create Requote Lists')
...
<ol class="breadcrumb breadcrumb-col-blue">
    ...
    <li class="active">Create Requote Lists</li>
</ol>
                    ...
                    <div class="header">
                        <div class="row clearfix">
                            <div class="col-xs-12 col-sm-12">
                                <h2>Create Requote Lists<span class="pull-right"><a href="{{ url('/admin/quote-list') }}" class="btn bg-green waves-effect" title="Back">
                                    <i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></span>
                                </h2>
                                <h4><p>Quote ({!! $getquote->quote_list_code !!})</p></h4>
                            </div>
                        </div>
                    </div>
                    {!! Form::model($quotelist, [
                        'url' => ['/admin/quote-list/do_requote', $quotelist->id],
                        'class' => 'form-horizontal',
                        'files' => true,
                    ]) !!}
                    <input type="hidden" name="id" class="form-control" value="{{ $quotelist->id }}" placeholder="">
                    <input type="hidden" name="requote_list_code" class="form-control" value="{{ $quotelist->requote_list_code }}" placeholder="">
                    <div class="body">
                        <div class="row">
                            ...
                            <div class="col-sm-6 col-xs-12">
                                <div class="form-float {{ $errors->has('quote_list_code') ? 'has-error' : ''}}">
                                    <label class="form-label" style="font-weight: 100; color: #aaa;">Requote Name <span style="font-size: 15px;color: red;line-height:15px;">*</span></label>
                                    <input type="text" name="quote_list_code" id="requote_list_code" class="form-control" value="@if(old('quote_list_code') != ''){{ old('quote_list_code') }}@else{{ $quotelist->quote_list_code }}@endif" placeholder="">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6 col-xs-12">
                                ...
                                <div class="form-float {{ $errors->has('date_out') ? 'has-error' : ''}}">
                                    ...
                                    <input name="date_out" type="text" id="date_out" class="datetimepicker form-control" value="{{ isset($quotelist->date_out) ? $quotelist->date_out : '' }}" placeholder="Please choose date & time..." style="margin-top: -4px;">
                            ...
                            <div class="col-sm-6 col-xs-12">
                                ...
                                <div class="form-float">
                                    ...
                                    <textarea name="note" id="note" class="form-control" rows="3">{{ $quotelist->note }}</textarea>
                                </div>
                            </div>
                            <div class="col-sm-6 col-xs-12">
                                <hr>
                            ...
                            <div class="col-sm-6 col-xs-12">
                                ...
                                <div class="form-float {{ $errors->has('term_condition_id') ? 'has-error' : ''}}">
                                    <label class="form-label" style="font-weight: 100; color: #aaa;">Term & Condition : <span style="font-size: 15px;color: red;line-height:15px;">*</span></label>
                ...
                <div class="body">
                    ...
                    <div class="form-group text-center">
                        @if(count($list_detail) > 0)
                            {!! Form::submit(isset($submitButtonText) ? $submitButtonText : 'Create Requote', ['class' => 'btn bg-green btn-sm btn-block waves-effect', 'style' => 'font-size:19px;']) !!}
                        @else
                            <h4><div style="color: red;"><i>*Please select the Prospect first</i></div></h4>
                            <input type="button" disabled="disabled" value="Create Requote" class="btn bg-green btn-sm btn-block waves-effect" style="font-size:19px;">
                        @endif
                    </div>
...
@push('script')
    <script>
        ...
        $('#prospect_sales_id').on('change', function(){
            ...
            var quote_list_id = "{{ $quotelist->id }}";
            var quote_list_code = $('#requote_list_code').val();
            ...
            $.ajax({
                ...
                success : function (data)
                {
                    if(data.data.choose_tax == 1){
                        ...
                        htmll = '<input type="text" name="tax" id="tax" value="0" class="form-control" maxlength="4" style="width: 60px;">';
                        $('#input_tax').html(htmll);
                        // selectTax += '<select name="choose_tax" id="choose_tax" class="form-control choose_tax">';
                        // selectTax += ' <option value="0">Dont use tax</option>';
                        // selectTax += ' <option value="1">Using tax</option>';
                        // selectTax += '</select>';
                        // $('#tax_select').html(selectTax);
                    }
                    $('#tex_rupiah').html(insertCommas(data.data.tax_price+'.00' ));
                    $('.after_tax').html(insertCommas(data.data.after_tax+'.00' ));
